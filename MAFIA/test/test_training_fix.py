...
def print_progress(message, newline=False):
    """Print progress message on single line (real-time update)."""
    end = "\n" if newline else ""
    print(f"\r{message:<120}", end=end, flush=True)
...
def test_environment_step_format():
    ...
    env = StockPortfolioEnv(
        config=config, rawdata=data, mode="train", stock_num=1, action_dim=1
    )

    # Test reset
    obs = env.reset()
    print_progress(
        f"Reset returned: obs shape {obs.shape if hasattr(obs, 'shape') else 'no shape'}",
        newline=True,
    )
    ...
    print_progress(f"Step returned {len(result)} values", newline=True)

    if len(result) == 5:
        obs, reward, terminated, truncated, info = result
        print_progress(
            "✓ Correct gymnasium format: (obs, reward, terminated, truncated, info)",
            newline=True,
        )
        print_progress(
            f"  terminated: {terminated}, truncated: {truncated}", newline=True
        )
    else:
        print_progress("✗ Unexpected return format", newline=True)
    ...
    while not terminated and not truncated:
        ...
        if len(result) == 5:
            obs, reward, terminated, truncated, info = result
            # Real-time single line update
            print_progress(
                f"  Step {step_count}: terminated={terminated}, truncated={truncated}"
            )
...
def test_checkpoint_cleanup_enabled():
    ...
    print(
        f"enable_checkpoint_cleanup: {getattr(callback, 'enable_checkpoint_cleanup', 'not set')}"
    )
    print(
        f"max_checkpoints_to_keep: {getattr(callback, 'max_checkpoints_to_keep', 'not set')}"
    )
