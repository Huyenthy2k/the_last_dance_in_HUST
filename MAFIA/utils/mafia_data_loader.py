...
class MAFIADataLoader:
    ...
    def __init__(self, config):
        ...
        """
        self.config = config
        self.use_features = config.use_features  # ['close', 'open', 'high', 'low', 'volume']
    ...
    def load_and_split_data(self, data: pd.DataFrame) -> dict:
        ...
        """
        print("[MAFIA] Loading data with lightweight preprocessing (no legacy features)...", flush=True)
        ...
        train_data = train_data.sort_values(['date', 'stock'], ascending=True, ignore_index=True)
        dataset_dict['train'] = train_data
        print(f"[MAFIA] Train data: {len(train_data)} rows, "
              f"dates: {train_data['date'].min()} to {train_data['date'].max()}", flush=True)
        ...
        if (self.config.valid_date_start is not None) and (self.config.valid_date_end is not None):
            ...
            valid_data = valid_data.sort_values(['date', 'stock'], ascending=True, ignore_index=True)
            dataset_dict['valid'] = valid_data
            print(f"[MAFIA] Valid data: {len(valid_data)} rows, "
                  f"dates: {valid_data['date'].min()} to {valid_data['date'].max()}", flush=True)
        else:
        ...
        if (self.config.test_date_start is not None) and (self.config.test_date_end is not None):
            ...
            test_data = test_data.sort_values(['date', 'stock'], ascending=True, ignore_index=True)
            dataset_dict['test'] = test_data
            print(f"[MAFIA] Test data: {len(test_data)} rows, "
                  f"dates: {test_data['date'].min()} to {test_data['date'].max()}", flush=True)
        else:
        ...
        print("[MAFIA] Data loading complete (lightweight mode).", flush=True)
        return dataset_dict
    ...
    def _process_fine_data(self, data: pd.DataFrame, split: str) -> dict:
        ...
        return {
            'fine_market': fine_market,
            'fine_stock': fine_stock
        }
    
    def _create_fine_market_data(self, data: pd.DataFrame, start_date, end_date) -> pd.DataFrame:
        """
        ...
        market_agg = data.groupby('date').agg({
            'close': 'mean',
            'open': 'mean',
            'high': 'mean',
            'low': 'mean',
        ...
        market_agg = pd.concat([market_agg, pd.DataFrame(windowed_cols, index=market_agg.index)], axis=1)
        
        # Add required columns for environment compatibility
        market_agg[f'mkt_{self.finefreq}_close'] = market_agg['close']
        market_agg[f'mkt_{self.finefreq}_ma'] = market_agg['close'].rolling(window=5, min_periods=1).mean()
        
        # Filter to split dates only
        fine_market = market_agg[
            (market_agg['date'] >= start_date) & 
            (market_agg['date'] <= end_date)
        ].copy()
        
        return fine_market
    
    def _create_fine_stock_data(self, data: pd.DataFrame, start_date, end_date) -> pd.DataFrame:
        """
        ...
        for stock in stock_lst:
            stock_data = data[data['stock'] == stock].sort_values('date', ascending=True, ignore_index=True)
            ...
            # Batch insert all windowed columns at once
            stock_data = pd.concat([stock_data, pd.DataFrame(windowed_cols, index=stock_data.index)], axis=1)
            
            # Add required columns for environment compatibility
            stock_data[f'stock_{self.finefreq}_close'] = stock_data['close']
            stock_data[f'stock_{self.finefreq}_ma'] = stock_data['close'].rolling(window=5, min_periods=1).mean()
        ...
        # Filter to split dates only
        fine_stock = fine_stock[
            (fine_stock['date'] >= start_date) & 
            (fine_stock['date'] <= end_date)
        ].copy()
    ...
    def _add_daily_returns(self, data: pd.DataFrame, lookback: int = 20) -> pd.DataFrame:
        """
        ...
        for stock in stock_lst:
            stock_data = data[data['stock'] == stock].sort_values('date', ascending=True).copy()
            ...
            if len(close_prices) <= 1:
                ...
                daily_returns = pd.Series(pct_changes).rolling(window=lookback, min_periods=1).mean().values
    ...
    def _add_moving_average(self, data: pd.DataFrame, window: int = 5) -> pd.DataFrame:
        ...
        for stock in stock_lst:
            ...
            close_prices = stock_data['close'].values
            ma_values = pd.Series(close_prices).rolling(window=window, min_periods=1).mean().values
