...
try:
    ...
    smart_print(
        "Warning: TA-Lib is not available. Technical indicators from TA-Lib will not work."
    )
    smart_print("Please install TA-Lib: https://github.com/TA-Lib/ta-lib-python")
    ...
    class DummyAbstract:
        class Function:
            def __init__(self, *args, **kwargs):
                raise ImportError(
                    "TA-Lib is not installed. Please install it to use technical indicators."
                )
...
class FeatureProcesser:
    ...
    def gen_feat(self, data):
        ...
        data.sort_values(
            ["stock", "date"], ascending=True, inplace=True, ignore_index=True
        )
        # ['date', 'stock', 'open', 'high', 'low', 'close', 'volume']
        self.rawColLst = list(data.columns)
        datax = copy.deepcopy(data)
        stock_lst = datax["stock"].unique()
        for indidx, sigIndicatorName in enumerate(
            list(self.config.tech_indicator_input_lst)
            + list(self.config.otherRef_indicator_lst)
        ):
            ...
            for sigStockName in stock_lst:
                ...
                dataSig.sort_values(
                    ["date"], ascending=True, inplace=True, ignore_index=True
                )
                if sigIndicatorName == "CHANGE":
                    ...
                    for change_feat in self.config.use_features:
                        ...
                        for widx in range(2, self.config.window_size + 1):
                            ...
                            if widx - 1 < len(cg_ay):
                                windowed = np.append(
                                    np.zeros(widx - 1), cg_ay[: -(widx - 1)], axis=0
                                )
                            else:
                            ...
                            if len(windowed) > data_len:
                                ...
                                windowed = np.append(
                                    windowed, np.zeros(data_len - len(windowed)), axis=0
                                )
                            temp["{}_w{}".format(change_feat, widx)] = windowed

                # {indicator name}-{window}-{output field}-{input field}
                elif (sigIndicatorName in self.config.tech_indicator_talib_lst) or (
                    sigIndicatorName in self.config.otherRef_indicator_lst
                ):
                    ...
                    if iname == "MA" and not TALIB_AVAILABLE:
                        ...
                        raise ImportError(
                            "TA-Lib is required for indicator {} but is not installed. Please install TA-Lib.".format(
                                sigIndicatorName
                            )
                        )
                    else:
                        ...
                        if len(indNameLst) == 1:
                            ...
                            raise ValueError(
                                "Unexpect indicator {}".format(sigIndicatorName)
                            )

                        if iname in ["OBV"]:
                            ind_val = indFunc(
                                dataSig[["open", "high", "low", "close", "volume"]]
                            )
                            ...
                            raise ValueError(
                                "Invalid input fields: {}".format(indFunc.input_names)
                            )
                        ...
                        if len(output_fields) == 1:
                            ...
                            if ofield is None:
                                if sigIndicatorName == "MACD":
                                    ...
                                    temp = {
                                        "AROONDOWN": np.array(ind_val["aroondown"]),
                                        "AROONUP": np.array(ind_val["aroonup"]),
                                    }
                                elif sigIndicatorName == "BBANDS":
                                    temp = {
                                        "BOLLUP": np.array(ind_val["upperband"]),
                                        "BOLLMID": np.array(ind_val["middleband"]),
                                        "BOLLLOW": np.array(ind_val["lowerband"]),
                                    }
                                else:
                                    temp = {
                                        sigIndicatorName: np.array(
                                            ind_val[sorted(list(ind_val.keys()))[0]]
                                        )
                                    }
                            else:
                    ...
                    raise ValueError(
                        "Please specify the category of the indicator: {}".format(
                            sigIndicatorName
                        )
                    )
        ...
        self.techIndicatorLst = sorted(
            list(
                set(cur_cols)
                - set(self.rawColLst)
                - set(self.config.otherRef_indicator_lst)
            )
        )
        return datax
    ...
    def scale_feat(self, data):
        ...
        if self.config.enable_cov_features:
            datax.sort_values(
                ["date", "stock"], ascending=True, inplace=True, ignore_index=True
            )
            ...
            for idx in range(self.config.cov_lookback, datax["date"].nunique()):
                ...
                sigPeriodClose = sigPeriodData.pivot_table(
        ...
        if (
            ...
            in self.config.otherRef_indicator_lst
        ):
            r_lst = []
            stockNo_lst = []
            date_lst = []
            ...
            for idx in range(self.config.dailyRetun_lookback, datax["date"].nunique()):
                sigPeriodData = datax.loc[
                    idx - self.config.dailyRetun_lookback : idx, :
                ][["date", "stock", "close"]]
                sigPeriodClose = sigPeriodData.pivot_table(
                    index="date", columns="stock", values="close"
                )
                sigPeriodClose.sort_values(["date"], ascending=True, inplace=True)
                sigPeriodReturn = sigPeriodClose.pct_change(
                    fill_method=None
                ).dropna()  # without percentage
                ...
                date_lst = date_lst + [datax.loc[idx, "date"].values[0]] * len(
                    sigStockName_lst
                )
            r_pd = pd.DataFrame(
                {
                    "date": date_lst,
                    "stock": stockNo_lst,
                    "DAILYRETURNS-{}".format(self.config.dailyRetun_lookback): r_lst,
                }
            )
        ...
        data_bftrain = copy.deepcopy(
            datax[datax["date"] < self.config.train_date_start][
                [
                    "date",
                    "stock",
                    "DAILYRETURNS-{}".format(self.config.dailyRetun_lookback),
        ...
        datax = copy.deepcopy(
            datax[
                (datax["date"] >= self.config.train_date_start)
                & (datax["date"] <= data_date_end)
            ]
        )
        datax.sort_values(
            ["date", "stock"], ascending=True, inplace=True, ignore_index=True
        ...
        for sigIndicatorName in self.techIndicatorLst:
            ...
            if (nan_cnt > 0) or (inf_cnt > 0):
                raise ValueError(
                    "Indicator: {}, nan count: {}, inf count: {}".format(
                        sigIndicatorName, nan_cnt, inf_cnt
                    )
                )
            if (
                (sigIndicatorName in ["CHANGELOGCLOSE", "cov"])
                or ("close_w" in sigIndicatorName)
                or ("open_w" in sigIndicatorName)
                or ("high_w" in sigIndicatorName)
                or ("low_w" in sigIndicatorName)
                or ("volume_w" in sigIndicatorName)
            ):
            ...
            train_ay = np.array(
                datax[
                    (datax["date"] >= self.config.train_date_start)
                    & (datax["date"] <= self.config.train_date_end)
                ][sigIndicatorName]
            )
            ind_mean = np.mean(train_ay)
            ind_std = np.std(train_ay, ddof=1)
            datax[sigIndicatorName] = (
                np.array(datax[sigIndicatorName]) - ind_mean
            ) / ind_std
        if self.config.enable_cov_features:
        ...
        cols_order = (
            list(self.rawColLst)
            + list(self.config.otherRef_indicator_lst)
            + list(sorted(self.techIndicatorLst))
        )
        datax = datax[cols_order]

        dataset_dict = {}
        train_data = copy.deepcopy(
            datax[
                (datax["date"] >= self.config.train_date_start)
                & (datax["date"] <= self.config.train_date_end)
            ]
        ...
        if (self.config.valid_date_start is not None) and (
            ...
            valid_data = copy.deepcopy(
                datax[
                    (datax["date"] >= self.config.valid_date_start)
                    & (datax["date"] <= self.config.valid_date_end)
                ]
            )
        ...
        if (self.config.test_date_start is not None) and (
            ...
            test_data = copy.deepcopy(
                datax[
                    (datax["date"] >= self.config.test_date_start)
                    & (datax["date"] <= self.config.test_date_end)
                ]
            )
    ...
    def process_finedata(self, data):
        ...
        fmd_train = copy.deepcopy(
            fine_mkt_data[
                (fine_mkt_data["date"] >= self.config.train_date_start)
                & (fine_mkt_data["date"] <= self.config.train_date_end)
            ]
        )
        ...
        if (
            hasattr(self.config, "finemkt_indicator_cols")
            and len(self.config.finemkt_indicator_cols) > 0
        ):
            norm_cols = [
                c for c in self.config.finemkt_indicator_cols if c in fmd_train.columns
            ]
            train_stats = {}
            for c in norm_cols:
        ...
        extra_train_data["market_index_feature_names"] = (
            self.config.market_index_feature_names
        )
        ...
        if len(common_dates) == 0:
            raise ValueError(
                "[Train, fine market] | No common dates between fine market data and daily data"
            )
        if (
            len(set(fmd_date_lst) - set(daily_date_lst)) > 0
            or len(set(daily_date_lst) - set(fmd_date_lst)) > 0
        ):
            smart_print(
                "Warning: [Train, fine market] | Date mismatch - using intersection. Fine market dates: {}, Daily dates: {}, Common: {}".format(
                    len(fmd_date_lst), len(daily_date_lst), len(common_dates)
                )
            )

            fsd_train = copy.deepcopy(
                fine_stock_data[
                    (fine_stock_data["date"] >= self.config.train_date_start)
                    & (fine_stock_data["date"] <= self.config.train_date_end)
                ]
            )
            fsd_train.sort_values(
                ["stock", "date"], ascending=True, inplace=True, ignore_index=True
            )
            ...
            if len(common_dates) == 0:
                raise ValueError(
                    "[Train, fine stock] | No common dates between fine stock data and daily data"
                )
            if (
                len(set(fsd_date_lst) - set(daily_date_lst)) > 0
                or len(set(daily_date_lst) - set(fsd_date_lst)) > 0
            ):
                smart_print(
                    "Warning: [Train, fine stock] | Date mismatch - using intersection. Fine stock dates: {}, Daily dates: {}, Common: {}".format(
                        len(fsd_date_lst), len(daily_date_lst), len(common_dates)
                    )
                )
        ...
        if (self.config.valid_date_start is not None) and (
            ...
            fmd_valid = copy.deepcopy(
                fine_mkt_data[
                    (fine_mkt_data["date"] >= self.config.valid_date_start)
                    & (fine_mkt_data["date"] <= self.config.valid_date_end)
                ]
            )
            fmd_valid.sort_values(
                ["date"], ascending=True, inplace=True, ignore_index=True
            )
            ...
            extra_valid_data["market_index_feature_names"] = (
                self.config.market_index_feature_names
            )
            ...
            if len(common_dates) == 0:
                raise ValueError(
                    "[Valid, fine market] | No common dates between fine market data and daily data"
                )
            if (
                len(set(fmd_date_lst) - set(daily_date_lst)) > 0
                or len(set(daily_date_lst) - set(fmd_date_lst)) > 0
                ...
                smart_print(
                    "Warning: [Valid, fine market] | Date mismatch - using intersection. Fine market dates: {}, Daily dates: {}, Common: {}".format(
                        len(fmd_date_lst), len(daily_date_lst), len(common_dates)
                    )
                )

            fsd_valid = copy.deepcopy(
                fine_stock_data[
                    (fine_stock_data["date"] >= self.config.valid_date_start)
                    & (fine_stock_data["date"] <= self.config.valid_date_end)
                ]
            )
            fsd_valid.sort_values(
                ["stock", "date"], ascending=True, inplace=True, ignore_index=True
            )
            ...
            if len(common_dates) == 0:
                raise ValueError(
                    "[Valid, fine stock] | No common dates between fine stock data and daily data"
                )
            if (
                len(set(fsd_date_lst) - set(daily_date_lst)) > 0
                or len(set(daily_date_lst) - set(fsd_date_lst)) > 0
                ...
                smart_print(
                    "Warning: [Valid, fine stock] | Date mismatch - using intersection. Fine stock dates: {}, Daily dates: {}, Common: {}".format(
                        len(fsd_date_lst), len(daily_date_lst), len(common_dates)
                    )
                )
        ...
        if (self.config.test_date_start is not None) and (
            ...
            extra_test_data = {}
            fmd_test = copy.deepcopy(
                fine_mkt_data[
                    (fine_mkt_data["date"] >= self.config.test_date_start)
                    & (fine_mkt_data["date"] <= self.config.test_date_end)
                ]
            )
            fmd_test.sort_values(
                ["date"], ascending=True, inplace=True, ignore_index=True
            )
            ...
            if len(fmd_date_lst) == 0:
                smart_print(
                    "Warning: [Test, fine market] | No fine market data in test period, creating from daily stock data"
                )
                ...
                for date in daily_date_lst:
                    ...
                    if len(date_data) > 0:
                        ...
                        market_row = {
                            "date": date,
                            "mkt_{}_close".format(self.config.finefreq): date_data[
                                "close"
                            ].mean(),
                            "mkt_{}_ma".format(self.config.finefreq): date_data[
                                "close"
                            ].mean(),  # Use close as MA fallback
                        }
                        ...
                        for feat in self.config.use_features:
                            for w in range(1, self.config.fine_window_size + 1):
                                market_row[
                                    "mkt_{}_{}_w{}".format(
                                        self.config.finefreq, feat, w
                                    )
                                ] = (
                                    date_data[feat].mean()
                                    if feat in date_data.columns
                                    else 0.0
                                )
            ...
            extra_test_data["market_index_feature_names"] = (
                self.config.market_index_feature_names
            )
            # Apply train-only normalization
            if "train_stats" in locals():
                for c, (mu, sigma) in train_stats.items():
                    if c in extra_test_data["fine_market"].columns:
                        extra_test_data["fine_market"][c] = (
                            extra_test_data["fine_market"][c] - mu
                        ) / sigma
            # Use intersection of dates to avoid mismatch
            common_dates = set(fmd_date_lst) & set(daily_date_lst)
            if len(common_dates) == 0:
                raise ValueError(
                    "[Test, fine market] | No common dates between fine market data and daily data after fallback"
                )
            if (
                len(set(fmd_date_lst) - set(daily_date_lst)) > 0
                or len(set(daily_date_lst) - set(fmd_date_lst)) > 0
                ...
                smart_print(
                    "Warning: [Test, fine market] | Date mismatch - using intersection. Fine market dates: {}, Daily dates: {}, Common: {}".format(
                        len(fmd_date_lst), len(daily_date_lst), len(common_dates)
                    )
                )

            fsd_test = copy.deepcopy(
                fine_stock_data[
                    (fine_stock_data["date"] >= self.config.test_date_start)
                    & (fine_stock_data["date"] <= self.config.test_date_end)
                ]
            )
            fsd_test.sort_values(
                ["stock", "date"], ascending=True, inplace=True, ignore_index=True
            )
            ...
            if len(fsd_date_lst) == 0:
                smart_print(
                    "Warning: [Test, fine stock] | No fine stock data in test period, creating from daily stock data"
                )
                # Create fine stock data from daily stock data
                test_daily = data["test"].copy()
                fine_stock_list = []
                for stock in test_daily["stock"].unique():
                    stock_data = test_daily[test_daily["stock"] == stock].sort_values(
                        "date"
                    )
                    for _, row in stock_data.iterrows():
                        fine_row = {
                            ...
                            "stock_{}_ma".format(self.config.finefreq): row[
                                "close"
                            ],  # Use close as MA fallback
                        }
                        # Add window features (simplified - use same value for all windows)
                        for feat in self.config.use_features:
                            for w in range(1, self.config.fine_window_size + 1):
                                ...
                                ] = row[feat] if feat in row.index else 0.0
            ...
            if len(common_dates) == 0:
                raise ValueError(
                    "[Test, fine stock] | No common dates between fine stock data and daily data after fallback"
                )
            if (
                len(set(fsd_date_lst) - set(daily_date_lst)) > 0
                or len(set(daily_date_lst) - set(fsd_date_lst)) > 0
                ...
                smart_print(
                    "Warning: [Test, fine stock] | Date mismatch - using intersection. Fine stock dates: {}, Daily dates: {}, Common: {}".format(
                        len(fsd_date_lst), len(daily_date_lst), len(common_dates)
                    )
                )
    ...
    def gen_market_feat(self, freq, daily_date_lst=None):
        ...
        isHasFineData = (
            ...
            and "{}_{}_index.csv".format(self.config.market_name, freq) in fpath
        )
        ...
        if fpath is None:
            ...
            if fpath is None:
                ...
                smart_print(
                    "No index data file found. Generating market features from stock data instead."
                )
                return self._gen_market_feat_from_stock_data(freq)
            smart_print(
                "Cannot find the {}-freq market data, will use 1d data instead.".format(
                    freq
                )
            )
        ...
        raw_data = pd.DataFrame(
            pd.read_csv(
                fpath, header=0, usecols=["date"] + list(self.config.use_features)
            )
        )
        ...
        raw_data = (
            raw_data.groupby(["date"]).mean().reset_index(drop=False, inplace=False)
        )
        ...
        if not TALIB_AVAILABLE:
            ...
            ma_ay = (
                pd.Series(raw_data["close"])
                .rolling(window=cur_winsize + 1, min_periods=1)
                .mean()
                .values
            )
        else:
        ...
        temp = {
            "date": np.array(raw_data["date"]),
            "mkt_{}_close".format(freq): np.array(raw_data["close"]),
            "mkt_{}_ma".format(freq): ma_ay,
        }
        for change_feat in self.config.use_features:
            ...
            for widx in range(2, cur_winsize + 1):
                temp["mkt_{}_{}_w{}".format(freq, change_feat, widx)] = np.append(
                    np.zeros(widx - 1), cg_ay[: -(widx - 1)], axis=0
                )
        ...
        try:
            ...
            high_vals = (
                np.array(raw_data["high"]) if "high" in raw_data.columns else close_vals
            )
            low_vals = (
                np.array(raw_data["low"]) if "low" in raw_data.columns else close_vals
            )
            vol_vals = (
                np.array(raw_data["volume"])
                if "volume" in raw_data.columns
                else np.ones_like(close_vals)
            )
            # SMA20 (already computed with cur_winsize+1; also add fixed 20)
            sma20 = (
                pd.Series(close_vals).rolling(window=20, min_periods=1).mean().values
            )
            # RSI14
            ...
            rs = np.divide(
                avg_gain,
                avg_loss,
                out=np.ones_like(avg_gain, dtype=float),
                where=avg_loss != 0,
            )
            rsi14 = 100 - (100 / (1 + rs))
            ...
            bb_width = (bb_up - bb_low) / (bb_mid.replace(0, np.nan)).replace(
                np.nan, 1.0
            )
            mkt_pd["mkt_{}_bb_width".format(freq)] = np.nan_to_num(
                bb_width.values, nan=0.0, posinf=0.0, neginf=0.0
            )
            # Stochastic %K/%D(14,3)
            rolling_high14 = (
                pd.Series(high_vals).rolling(window=14, min_periods=1).max().values
            )
            rolling_low14 = (
                pd.Series(low_vals).rolling(window=14, min_periods=1).min().values
            )
            stoch_k = (
                np.divide(
                    close_vals - rolling_low14,
            ...
            cci20 = (typical_price - sma_tp20) / (
                0.015 * md20.replace(0, np.nan)
            ).replace(np.nan, 1.0)
            mkt_pd["mkt_{}_cci20".format(freq)] = np.nan_to_num(
                cci20.values, nan=0.0, posinf=0.0, neginf=0.0
            )
            ...
            rolling_peak60 = (
                pd.Series(close_vals).rolling(window=60, min_periods=1).max().values
            )
            drawdown60 = np.divide(
                rolling_peak60 - close_vals,
                rolling_peak60,
                out=np.zeros_like(close_vals),
            ...
            sma60 = (
                pd.Series(close_vals).rolling(window=60, min_periods=1).mean().values
            )
            regime = sma20 - sma60
            mkt_pd["mkt_{}_regime_sma20_60".format(freq)] = regime
        except Exception as e:
            smart_print(
                f"Warning: Unable to compute market indicators (SMA/RSI/ATR): {e}"
            )

        if freq == "60m":
            if isHasFineData:
                mkt_pd["time"] = mkt_pd["date"].apply(
                    lambda x: x.strftime("%H:%M:%S")
                )  # Get hour-min-sec
                mkt_pd = mkt_pd[
                    mkt_pd["time"]
                    == self.config.market_close_time[self.config.market_name]
                ][
                    ["date"]
                    + self.config.finemkt_feat_cols_lst
                    + ["mkt_{}_ma".format(freq), "mkt_{}_close".format(freq)]
                ]  # Extract the datapoint of the market close time.
                mkt_pd["date"] = mkt_pd["date"].apply(
                    lambda x: x.strftime("%Y-%m-%d")
                )  # Convert Year-Month-Day hh:mm:ss to Year-Month-Day
    ...
    def _gen_market_feat_from_stock_data(self, freq):
        ...
        if stock_fpath is None:
            raise ValueError(
                f"Cannot load stock data file to generate market features. {error_msg}"
            )
        ...
        raw_stock_data = pd.DataFrame(
            pd.read_csv(
                stock_fpath,
                header=0,
                usecols=["date", "stock"] + list(self.config.use_features),
            )
        )
        ...
        raw_data = (
            raw_stock_data.groupby(["date"])[self.config.use_features]
            .mean()
            .reset_index()
        )
        raw_data.sort_values(["date"], ascending=True, inplace=True, ignore_index=True)
    ...
    def gen_fine_stock_feat(self, daily_date_lst=None):
        ...
        if self.config.stock_data_file:
            ...
            fpath = os.path.join(
                self.config.dataDir,
                "{}_{}_{}.csv".format(
                    self.config.market_name, self.config.topK, self.config.finefreq
                ),
            )
            isHasFineData = True
        ...
        if not os.path.exists(fpath):
            ...
            smart_print(
                "Cannot find the {}-freq stock data, will use main stock data file instead.".format(
                    self.config.finefreq
                )
            )
        else:
            ...
            if not is_valid:
                ...
                if fpath is None:
                    raise ValueError(
                        f"Cannot load fine stock data file. Validation failed: {error_msg}"
                    )
                smart_print(
                    "Fine stock data file validation failed, will use main stock data file instead."
                )
        ...
        for stock_id in stock_lst:
            ...
            if not TALIB_AVAILABLE:
                ...
                ma_ay = (
                    pd.Series(dataSig["close"])
                    .rolling(window=self.config.fine_window_size + 1, min_periods=1)
                    .mean()
                    .values
                )
            else:
                ma_ay = ma_func(
                    np.array(dataSig["close"]),
                    timeperiod=self.config.fine_window_size + 1,
                )

            temp = {
                "date": np.array(dataSig["date"]),
                "stock_{}_close".format(self.config.finefreq): np.array(
                    dataSig["close"]
                ),
            ...
            output_cols = (
                ["date"]
                + self.config.finestock_feat_cols_lst
                + [
                    "stock_{}_ma".format(self.config.finefreq),
                    "stock_{}_close".format(self.config.finefreq),
                ]
            )
            if self.config.is_gen_dc_feat:
                dc_events = dc_feature_generation(
                    data=np.array(dataSig["close"]),
                    dc_threshold=self.config.dc_threshold[0],
                )
            ...
            for change_feat in self.config.use_features:
                ...
                if len(feat_vals) <= 1:
                    ...
                    cg_ay = np.append([0], cg_ay, axis=0)
                cg_ay = cg_ay * self.config.feat_scaler
                temp["stock_{}_{}_w{}".format(self.config.finefreq, change_feat, 1)] = (
                    cg_ay
                )
                for widx in range(2, self.config.fine_window_size + 1):
                    temp[
                        "stock_{}_{}_w{}".format(
                            self.config.finefreq, change_feat, widx
                        )
                    ] = np.append(np.zeros(widx - 1), cg_ay[: -(widx - 1)], axis=0)

            temp = pd.DataFrame(temp)
            if isHasFineData:
                temp["time"] = temp["date"].apply(
                    lambda x: x.strftime("%H:%M:%S")
                )  # Get hour-min-sec
                temp = temp[
                    temp["time"]
                    == self.config.market_close_time[self.config.market_name]
                ][output_cols]  # Extract the datapoint of the market close time.
