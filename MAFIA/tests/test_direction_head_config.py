...
def test_config_focal_alpha():
    """Test 2: Verify focal loss alpha = [1.65, 0.70, 1.0]"""
    from config import Config

    config = Config()
    expected_alpha = [1.65, 0.70, 1.0]
    ...
    print("✓ Test 2 PASSED: focal_alpha = [1.65, 0.70, 1.0]")
...
def test_direction_head_bias_init():
    ...
    # Expected bias values (log-priors for Bear=19.1%, Side=46.0%, Bull=34.9%)
    expected_bias = th.tensor([-1.655, -0.777, -1.053])
    ...
    print("✓ Test 3 PASSED: DirectionHead bias = [-1.655, -0.777, -1.053]")
...
def test_initial_softmax_distribution():
    ...
    # Expected distribution: Bear=19.1%, Side=46.0%, Bull=34.9%
    expected_probs = th.tensor([0.191, 0.460, 0.349])
